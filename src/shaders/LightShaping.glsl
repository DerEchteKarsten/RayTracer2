...
LightShaping unpackLightShaping(RAB_LightInfo lightInfo)
{
    LightShaping shaping;
    shaping.isSpot = uint((lightInfo.colorTypeAndFlags & kPolymorphicLightShapingEnableBit) != 0);
    ...
    shaping.iesProfileIndex = int((lightInfo.colorTypeAndFlags & kPolymorphicLightIesProfileEnableBit) == 0 ? lightInfo.iesProfileIndex : -1);
    return shaping;
}
...
float evaluateIesProfile(int profileIndex, float3 emissionDirection_, float3 lightPrimaryAxis)
{
    ...
    // const float normAngle = angle / RTXDI_PI;

    // const float tangentAngle = atan2(emissionDirection.y, emissionDirection.x);
    // const float normTangentAngle = tangentAngle * .5f / RTXDI_PI + .5f;
...
float3 evaluateLightShaping(LightShaping shaping, float3 surfacePosition, float3 lightSamplePosition)
{
    ...
    return vec3(softSpotlight * iesMultiplier);
}
...
float3 getConeVertexForSphericalSource(float3 sphereCenter, float sphereRadius, float3 coneAxis, float coneHalfAngle)
{
    ...
    float sinHalfAngle = sin(min(coneHalfAngle, RTXDI_PI * 0.5));
...
bool testSphereConeIntersection(float3 coneVertex, float3 coneAxis, float coneHalfAngle, float3 sphereCenter, float sphereRadius)
{
    ...
    float invDistanceFromConeVertexToSphereCenter = 1.0 / (distanceFromConeVertexToSphereCenter);
...
float getShapingFluxFactor(LightShaping shaping)
{
    ...
    solidAngleOverTwoPi *= mix(1.0, 0.5, shaping.cosConeSoftness);
