...
float RTXDI_ReGIR_GetJitterScale(ReGIR_Parameters params, vec3 worldPos)
{
    return params.commonParams.samplingJitter * params.commonParams.cellSize;
}

int RTXDI_ReGIR_WorldPosToCellIndex(ReGIR_Parameters params, vec3 worldPos)
{
    const vec3 gridCenter = vec3(params.commonParams.centerX, params.commonParams.centerY, params.commonParams.centerZ);
    const int3 gridCellCount = int3(params.gridParams.cellsX, params.gridParams.cellsY, params.gridParams.cellsZ);
    const vec3 gridOrigin = gridCenter - vec3(gridCellCount) * (params.commonParams.cellSize * 0.5);
...
bool RTXDI_ReGIR_CellIndexToWorldPos(ReGIR_Parameters params, int cellIndex, out vec3 cellCenter, out float cellRadius)
{
    const vec3 gridCenter = vec3(params.commonParams.centerX, params.commonParams.centerY, params.commonParams.centerZ);
    const int3 gridCellCount = int3(params.gridParams.cellsX, params.gridParams.cellsY, params.gridParams.cellsZ);
    const vec3 gridOrigin = gridCenter - vec3(gridCellCount) * (params.commonParams.cellSize * 0.5);
    ...
    {
        cellCenter = vec3(0.0, 0.0, 0.0);
        cellRadius = 0.0;
        return false;
    }

    cellCenter = (vec3(cellPosition) + 0.5) * params.commonParams.cellSize + gridOrigin;
...
float RTXDI_ReGIR_GetJitterScale(ReGIR_Parameters params, vec3 worldPos)
{
    const vec3 onionCenter = vec3(params.commonParams.centerX, params.commonParams.centerY, params.commonParams.centerZ);
    const vec3 translatedPos = worldPos - onionCenter;
...
bool RTXDI_ReGIR_CellIndexToWorldPos(ReGIR_Parameters params, int cellIndex, out vec3 cellCenter, out float cellRadius)
{
    ...
    vec3 cellCorner = RTXDI_SphericalToCartesian(layerOuterRadius, azimuth, elevation);
...
vec3 RTXDI_VisualizeReGIRCells(ReGIR_Parameters params, vec3 worldPos)
{
